...
class ConditionalEmbedding(nn.Module):
    def __init__(self, params_dim, dim):
        super().__init__()
        self.condEmbedding = nn.Sequential(
            nn.Linear(params_dim, dim),
            Swish(),
            nn.Linear(dim, dim),
        )
...
class UNet(nn.Module):
    def __init__(self, T, params_dim, ch, ch_mult, num_res_blocks, dropout):
        super().__init__()
        tdim = ch * 4
        self.time_embedding = TimeEmbedding(T, ch, tdim)
        self.cond_embedding = ConditionalEmbedding(params_dim, tdim)
